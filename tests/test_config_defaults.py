...
config_defaults_path = os.path.join(
    os.path.dirname(__file__), "..", "src", "lmpipeline", "utils", "config_defaults.py"
)
spec = importlib.util.spec_from_file_location("config_defaults", config_defaults_path)
if spec and spec.loader:
    config_defaults_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(config_defaults_module)
    ConfigDefaults = config_defaults_module.ConfigDefaults
else:
    raise ImportError("Could not load config_defaults module")
...
class TestConfigDefaults:
    ...
    def test_ensure_directory_exists_permission_error(self):
        ...
        # Patch the Path.mkdir method directly on the module
        with patch.object(
    ...
    def test_apply_storage_defaults_no_output_dir(self):
        ...
        config_dict = {"model_name_or_path": "test-model", "stages": ["sft"]}
    ...
    def test_get_fallback_directory_all_fail(self):
        ...
        with patch.object(
            ConfigDefaults, "ensure_directory_exists", return_value=False
        ):
            result = ConfigDefaults.get_fallback_directory(preferred_dir)
            assert result == "."
    ...
    def test_apply_all_defaults_integration(self):
        ...
        config_dict = {
            ...
            "convert_to_gguf": True,
            "gguf_quantization": "q4_0",
            "stage_configs": {"sft": {"use_4bit": True}},
        }
